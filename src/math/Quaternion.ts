 ...
 * mathematically defined as Q = x*i + y*j + z*k + w, where (i,j,k)
 * are imaginary basis vectors. (x,y,z) can be seen as a vector related
 * to the axis of rotation, while the real multiplier, w, is related to
...
export class Quaternion {
  ...
  private sfv_t1 = new Vec3();
  private sfv_t2 = new Vec3();
  ...
  setFromVectors(u: Vec3, v: Vec3) {
    if (u.isAntiparallelTo(v)) {
      const t1 = this.sfv_t1;
      const t2 = this.sfv_t2;
   ...
   * page describes: http://www.euclideanspace.com/maths/standards/index.htm
   * @method toEuler
   * @param {Vec3} target
   ...
   */
  setFromEuler(x: number, y: number, z: number, order: string = 'XYZ') {
